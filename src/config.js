...
function validateConfig(env = process.env) {
  ...
  const required = production ? ['DATABASE_URL', 'ADMIN_API_KEY', 'AUTH_TOKEN_SECRET', 'CORS_ORIGINS'] : [];
  ...
  const origins = String(env.CORS_ORIGINS || '').split(',').map((x) => x.trim()).filter(Boolean);
